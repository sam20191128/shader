Shader "AP01/L20/ClockAnim" //Shader路径名
{
    Properties //材质面板参数
    {
    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            void RotateZWithOffset(float angle, float offset, float mask, inout float3 vertex) {
                // 把遮罩顶点移到物体中心,offset是与模型坐标原点的距离，中心点移到原点
            ...
            {
                // 准备向量
                float3 nDirTS = UnpackNormal(tex2D(_NormTex, i.uv0)).rgb;// 采样法线纹理并解码 切线空间nDir
                ...
                float3 var_Cubemap = texCUBElod(_Cubemap, float4(vrDirWS, lerp(_CubemapMip, 0.0, var_SpecTex.a))).rgb;// 采样Cubemap
